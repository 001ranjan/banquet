...
if(globalVar.page=='checkout'){
 paymentInfo();  
  ...
  $("#check_out_date").on("change", function () {
      globalVar.checkOutDate = $(this).val();
      var endDate = moment(globalVar.checkOutDate, "YYYY.MM.DD");
      var startDate = moment(globalVar.startDate, "YYYY.MM.DD");
      
      // Calculate the difference in days and ensure a minimum of 1 day
      const diff = endDate.diff(startDate, 'days');
      globalVar.durationOfStayDays = Math.max(diff + 1, 1); // +1 to include the start date

      // Update the duration in the input field
      $('#duration_of_stay').val(globalVar.durationOfStayDays);

      // Update durationOfStay for each no_swapped_room
      $('.no_swapped_room').each(function () {
          $(this).text(globalVar.durationOfStayDays);
      });

      // Call the paymentInfo function
      paymentInfo();
  });
  ...
  $("#duration_of_stay").on("keyup click", function () {
      ...
      globalVar.durationOfStayDays = inputVal > 0 ? inputVal : 1;

      // Reflect the corrected value in the input field
      $(this).val(globalVar.durationOfStayDays);
...
if(globalVar.page=='room_reservation_add'){
  ...
  globalVar.durationOfStayDays = 1;
  ...
  $("#check_out_date").on("change",function(){
      ...
      globalVar.durationOfStayDays = Math.max(endDate.diff(startDate, 'days') + 1, 1);
      // Update the input field with the calculated duration
      $('#duration_of_stay').val(globalVar.durationOfStayDays);
      paymentInfo();
  });
...
$(document).ready(function () {
  // Add Special Row
  $(document).on('click', '.add-special-row', function () {
    // Clone the hidden template row
    var html = $(".colne_special_row_elem").html();
    if (html) {
      // Append the row and ensure it's visible
      $(".special_row_parent").append(html);
      $(".special_row_parent .special_row_elem:last").show();
    } else {
      console.error("Template row not found or empty.");
    }
  });

  // Delete Special Row
  $(document).on('click', '.delete-special-row', function () {
    // Remove the clicked row
    $(this).closest('.special_row_elem').remove();
  });
});
